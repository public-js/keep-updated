...
function updatePackages({
    ...
    for (const packageName of packagesList) {
        if (deps[packageName]) {
            ...
            process.stdout.write(`Updating dependency '${packageName}@${version}'` + '\n');
            updated[packageName] = { version, type: 'dependencies' };
            execSync(`${pmc.add} ${packageName}@${version}`, { stdio: [] });
        } else if (devDeps[packageName]) {
            const version = devDeps[packageName];
            process.stdout.write(`Updating devDependency '${packageName}@${version}'` + '\n');
            updated[packageName] = { version, type: 'devDependencies' };
            execSync(`${pmc.addDev} ${packageName}@${version}`, { stdio: [] });
        } else {
            process.stdout.write(`Dependency '${packageName}' not found in package.json.` + '\n');
        }
    }
